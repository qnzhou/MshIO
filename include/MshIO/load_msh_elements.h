...
void load_elements_ascii(std::istream& in, MshSpec& spec, std::ostream& log_out,
    ...
    for (size_t i=0; i<elements.num_elements; i++) {
        ...
        elements.min_element_tag = std::min(elements.min_element_tag, static_cast<size_t>(element_num));
        elements.max_element_tag = std::max(elements.max_element_tag, static_cast<size_t>(element_num));
...
void load_elements_binary(std::istream& in, MshSpec& spec, std::ostream& log_out,
    ...
    while (num_processed_elements != elements.num_elements) {
        ...
        if (num_processed_elements > elements.num_elements) {
            throw std::ios::failure("Inconsistent element count detected!");
        }
    }
